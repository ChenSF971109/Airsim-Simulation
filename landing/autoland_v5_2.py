...
*The system only use the PBVS data for navigation
...
ws.write(0, 7, label = 'UAV_vx')
ws.write(0, 8, label = 'UAV_vy')
ws.write(0, 9, label = 'land_x')
ws.write(0, 10, label = 'land_y')
row = 1
...
client.moveToPositionAsync(-24, 18, -25, 5).join() #(-20, 16, -18, 5)
time.sleep(10)
...
def data_processing():
    ...
    while UAV_position_z_val > safe_height:
        ...
        if math.sqrt(PBVS_x*PBVS_x + PBVS_y*PBVS_y) > 2:
            offset_x, offset_y, pixel_length, theta = aruco_detection('../Appdata/Local/Temp/airsim_drone/0.png', image_width, image_height)
        else:
...
def PTZ(Kp = 0.0005, Ki = 0.00002, Kd = 0.00003, dt=0.2):
    ...
    global camera_pitch, camera_yaw
    while math.sqrt(PBVS_x*PBVS_x + PBVS_y*PBVS_y) > 2:
        lock.acquire()
...
def fly(): 
    ...
    while UAV_position_z_val > safe_height: 
        ...
        if math.sqrt(PBVS_x*PBVS_x + PBVS_y*PBVS_y) > 2:
	        UAV_vx, UAV_vy, UAV_vz = speed_calculation(PBVS_x, PBVS_y, UAV_position_z_val, safe_height, vmax, vzmax)
        else:
        ...
        ws.write(row, 7, label = UAV_vx)
        ws.write(row, 8, label = UAV_vy)	
        row = row + 1
        lock.release()
        time.sleep(0.1)
...
ws.write(1, 9, label = land_x)
ws.write(1, 10, label = land_y)
