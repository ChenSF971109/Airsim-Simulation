...
*仅使用PBVS数据与IMU数据融合得到的相对位置信息进行导航，同时系统还加入了GPS干扰
...
*Only use the relative position information obtained from the fusion of the PBVS data and the IMU data for navigation, and GPS interference is added to the system at the same time
...
ws.write(0, 7, label = 'PBVS_IMU_x')
ws.write(0, 8, label = 'PBVS_IMU_y')
ws.write(0, 9, label = 'fusion_x')
ws.write(0, 10, label = 'fusion_y')
ws.write(0, 11, label = 'UAV_vx')
ws.write(0, 12, label = 'UAV_vy')
ws.write(0, 13, label = 'PBVS_IMU_flag')
ws.write(0, 14, label = 'land_x')
ws.write(0, 15, label = 'land_y')
row = 1
...
def data_processing():
    ...
    global PBVS_IMU_x_K, PBVS_IMU_y_K
...
def fly(): 
    ...
    while UAV_position_z_val > safe_height: 
        ...
        ws.write(row, 7, label = PBVS_IMU_x)
        ws.write(row, 8, label = PBVS_IMU_y)
        ws.write(row, 9, label = fusion_x)
        ws.write(row, 10, label = fusion_y)
        ws.write(row, 11, label = UAV_vx)
        ws.write(row, 12, label = UAV_vy)
        ws.write(row, 13, label = PBVS_IMU_flag)		
        row = row + 1
        lock.release()
        time.sleep(0.1)
...
ws.write(1, 14, label = land_x)
ws.write(1, 15, label = land_y)
